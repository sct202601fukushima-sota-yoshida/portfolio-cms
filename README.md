...
| マイグレーション | Flyway | 11.7.2 | SQL ネイティブ、V1〜V7 でスキーマ + シードを管理 |
...
34 テスト（Service 21 + Controller 11 + Context load 1 + その他 1）。Service 層は Mockito / 純粋ユニットテスト、Controller 層は `@WebMvcTest` スライスで高速に実行される。`MarkdownService` は太字 / リンク / リスト / softbreak / `<script>` エスケープ / `null` 入力など計 11 ケースで境界条件を網羅。
